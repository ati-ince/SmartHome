function varargout = EnergyMeasurement_v1(varargin)
%ati' nin malidir.......
% Last Modified by GUIDE v2.5 04-Nov-2013 01:14:14
%%
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @EnergyMeasurement_v1_OpeningFcn, ...
                   'gui_OutputFcn',  @EnergyMeasurement_v1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%%
% --- Executes just before EnergyMeasurement_v1 is made visible.
function EnergyMeasurement_v1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to EnergyMeasurement_v1 (see VARARGIN)
handles.degisken=1;
handles.buffer=cell((handles.degisken),1);
%bir digeri
handles.degisken_HEX=1;
handles.buffer_HEX=cell((handles.degisken_HEX),2);
%% burasý da gonder - al icin
handles.degisken_sending=1;
handles.buffer_sending=cell((handles.degisken_sending),1);%burada asiri dosya sismesini onleyelim,tekrar tek elemanli cellden baslasin iþe                                             
%%%%%%%%%%
...
% Choose default command line output for EnergyMeasurement_v1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
% UIWAIT makes EnergyMeasurement_v1 wait for user response (see UIRESUME)
...
function varargout = EnergyMeasurement_v1_OutputFcn(hObject, eventdata, handles) 
...
guidata(hObject,handles);
%
function data_sending(handles)
Sending_Cirrus_Data_Array_uint8=handles.Sending_Cirrus_Data_Array_uint8;
TxQueue=uint32(0);
num_tx=uint32(0);
if isempty(Sending_Cirrus_Data_Array_uint8) || (~(length(Sending_Cirrus_Data_Array_uint8)))%eger bos ise sona git bura son , hiçbirþey gonderilmiyor, uzunluguna da bakýyor tabi uzunluk sifir ise ayni sey VEYA ' ladik sonucta
else
%% bos degilse eger data gonder
  if length(Sending_Cirrus_Data_Array_uint8)==1
      Sending_Cirrus_Data_Array_uint8= uint8([0 Sending_Cirrus_Data_Array_uint8]);%nedense boy = 1 olunca gondermiyor
  end
             ...
             if ((status_Tx_Available==handles.FT_OK)&&(TxQueue==0))
               ...
               %
               %pause(0.1);%100 ms bekle
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data_reading(handles)
rx_available=uint32(0);  % TxQueue=uint32(0);
num_rx=uint32(0);% num_tx=uint32(0);              
[status_rx,rx_available] =GetRxBytesAvailable(handles.PortOBJ, rx_available);
rx_ava_buff=rx_available;
Rx_Data_Byte= NET.createArray('System.Byte',10);% for buffer (ZigBee ve xtender icin max 256 byte buffer yeterli)
%%
tic;
while(true)%sonsuz dongu icinde kontrol edecegiz
    zaman=toc;
                                        ...
                                        %% Burasi bizim mekan
                                            %%
                                                for i=1:rx_available
                                                          xXx=uint8 ( Rx_Data_Byte(i) ); %Bunun turunu ayar cekilebilecek hale getirelim
                                                          %handles.buffer{handles.degisken,1}= uint8 ( Rx_Data_Byte(i) )  ; %normalde uint32 felen mý ne de bunu dusenlemeliyiz
                                                          handles.buffer_sending{handles.degisken_sending,1}= dec2hex(xXx);
                                                          %handles.buffer{handles.degisken,1}= dec2bin(xXx);
                                                          handles.degisken_sending=handles.degisken_sending+1;
                                                end
                                                %handles.degisken=handles.degisken+1;%Bu
                                                %da iki data arasina bosluk
                                                %icun.... Simdilik kotu
                                                %gorundugunu dusunuyorum
                                                set(handles.listbox2, 'String', handles.buffer_sending);%alinan datanin gorunmesi icin      
                                                rx_available=uint32(0); 
                                 %pause(0.01);
...
%set(handles.listbox2, 'String', handles.buffer_sending);%alinan datanin gorunmesi icin 
%% Burada aldigimiz degeri cekelim
%Bunu get ile listbox 2 den cektirecegim ama seri hale getirdikten sonra
% if ~isempty(handles.buffer_sending)
% % for i=1:length(handles.buffer_sending)
% %     data(i)=  (hex2dec( handles.buffer_sending{i}));
% % end
% % set(handles.listbox2,'UserData',data );
% set(handles.listbox2,'UserData',handles.buffer_sending );

%set(handles.listbox1,'string',data );%sadece ekranda ne var ne yok diye bakmak amaci ile
%% burada da sifirlayalim
handles.degisken_sending=1;
handles.buffer_sending=cell((handles.degisken_sending),1);
handles.buffer_sending=0;%sifirla
% else
%     set(handles.listbox2,'UserData',0 );
% end
%kayit et
%guidata(handles);
%pause(0.1);%biraz bekleme olsun tabi
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% --- Executes on button press in pushbutton_Result_Vrms.
function pushbutton_Result_Vrms_Callback(hObject, eventdata, handles)
handles.Sending_Cirrus_Data_Array_uint8=uint8([144 7]);%Vrms
data_sending(handles);
data_reading(handles);
% data=get(handles.listbox2,'UserData');
% toplam=0;
% for i=length(data) :1
% toplam=toplam+data(i) * (256^(2*i-2));
% end
% %handles.toplam_Vrms=Calibration_Vrms(toplam);
% handles.toplam_Vrms=(toplam);
%% deðiþik
data=hex2dec(get(handles.listbox2,'string'));
toplam=0;
...
set(handles.text_Result_Vrms,'string',handles.toplam_Vrms);
set(handles.listbox1,'string',data);
% hObject    handle to pushbutton_Result_Vrms (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on button press in pushbutton_Result_Irms.
function pushbutton_Result_Irms_Callback(hObject, eventdata, handles)
handles.Sending_Cirrus_Data_Array_uint8=uint8([144 6]);%Irms
data_sending(handles);
data_reading(handles);
% data=get(handles.listbox2,'UserData');
% toplam=0;
...
% toplam=toplam+data(i) * (256^(2*i-2));
% end
% %handles.toplam_Irms=Calibration_Irms(toplam);
% handles.toplam_Irms=(toplam);
data=hex2dec(get(handles.listbox2,'string'));
toplam=0;
for i=length(data) : (-1) : 1
toplam=toplam+data(i) * (256^(2*i-2));
...
handles.toplam_Irms=Calibration_Irms(toplam);
set(handles.text_Result_Irms,'string',handles.toplam_Irms);
set(handles.listbox1,'string',data);
% hObject    handle to pushbutton_Result_Irms (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on button press in pushbutton_Result_P.
function pushbutton_Result_P_Callback(hObject, eventdata, handles)
handles.Sending_Cirrus_Data_Array_uint8=uint8([144 7]);%Vrms
data_sending(handles);
data_reading(handles);
% data_V=get(handles.listbox2,'UserData');
% toplam_V=0;
% for i=length(data_V) :1
% toplam_V=toplam_V+data_V(i) * (256^(2*i-2));
% end
% %toplam_V=Calibration_Vrms(toplam_V);
% toplam_V=(toplam_V);
data=hex2dec(get(handles.listbox2,'string'));
toplam=0;
for i=length(data) : (-1) : 1
...
data_sending(handles);
data_reading(handles);
% data_I=get(handles.listbox2,'UserData');
% toplam_I=0;
% for i=length(data_I) :1
% toplam_I=toplam_I+data_I(i) * (256^(2*i-2));
% end
% %toplam_I=Calibration_Irms(toplam_I);
% toplam_I=(toplam_I);
data=hex2dec(get(handles.listbox2,'string'));
toplam=0;
for i=length(data) : (-1) : 1
...
handles.toplam_Irms=Calibration_Irms(toplam);
set(handles.text_Result_P,'string',((handles.toplam_Vrms*handles.toplam_Irms)/1000) );% hObject    handle to pushbutton_Result_P (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
